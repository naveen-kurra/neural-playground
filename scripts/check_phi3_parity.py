...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare exported Phi-3 model.py against native Transformers Phi-3.")
    parser.add_argument("--model-file", required=True, help="Path to generated Phi-3 model.py")
...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("generated_phi3_model", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to import generated module from {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generated_phi3_model"] = module
...
def main() -> int:
    ...
    hf_config = HFPhi3Config(**model_config, **hf_only_config)
    hf_model = HFPhi3ForCausalLM(hf_config).eval()

    generated_config = generated.Phi3Config(
        **model_config,
        head_dim=args.hidden_size // args.heads,
    )
    generated_model = generated.Phi3ForCausalLM(generated_config).eval()
