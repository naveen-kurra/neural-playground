...
export type MistralConfigInput = {
  name?: string;
  modelId?: string;
  vocabSize?: number;
  hiddenSize?: number;
...
export function buildMistralArchitectureSpec(input: MistralConfigInput = {}): MistralArchitectureSpec {
  const hiddenSize = input.hiddenSize ?? 4096;
  const numAttentionHeads = input.numAttentionHeads ?? 32;
  const numKeyValueHeads = input.numKeyValueHeads ?? 8;
  const headDim = input.headDim ?? Math.floor(hiddenSize / numAttentionHeads);

  const config = {
    vocabSize: input.vocabSize ?? 32768,
    hiddenSize,
    intermediateSize: input.intermediateSize ?? 14336,
    numHiddenLayers: input.numHiddenLayers ?? 32,
    numAttentionHeads,
    numKeyValueHeads,
    headDim,
    hiddenActivation: input.hiddenActivation ?? "silu",
    maxPositionEmbeddings: input.maxPositionEmbeddings ?? 32768,
    ...
    slidingWindow: input.slidingWindow ?? null
  };
  ...
  const embeddingOp: MistralEmbeddingOp = {
  ...
  const blocks: MistralBlockOp[] = Array.from({ length: config.numHiddenLayers }, (_, index) => {
    const inputRef = index === 0 ? embeddingOutput : `hidden.block_${index - 1}`;
    const outputRef = `hidden.block_${index}`;
    return {
      kind: "llama_block",
      id: `mistral_block_${index}`,
      input: inputRef,
      output: outputRef,
      hiddenSize: config.hiddenSize,
      ...
      expandTo: {
        inputNorm: {
          kind: "llama_rms_norm",
          id: `mistral_block_${index}.input_layernorm`,
          input: inputRef,
        ...
        attention: {
          ...
          id: `mistral_block_${index}.self_attn`,
        ...
        attentionResidual: {
          kind: "residual_add",
          id: `mistral_block_${index}.attn_residual`,
          inputs: [inputRef, `hidden.block_${index}.attn_out`],
          output: `hidden.block_${index}.attn_residual`
        },
        postAttentionNorm: {
          kind: "llama_rms_norm",
          id: `mistral_block_${index}.post_attention_layernorm`,
        ...
        mlpResidual: {
          kind: "residual_add",
          id: `mistral_block_${index}.mlp_residual`,
  ...
  const finalNormOp: MistralFinalNormOp = {
    kind: "llama_final_rms_norm",
    id: "mistral_final_norm",
  ...
  const lmHeadOp: MistralLmHeadOp = {
    kind: "llama_lm_head",
    id: "mistral_lm_head",
