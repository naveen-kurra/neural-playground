import { renderActivationRegistry } from "../../shared/python-snippets";

export function renderMistralPrimitives(): string {
  return [
    renderActivationRegistry("mistral"),
...
def repeat_kv(hidden_states: torch.Tensor, num_key_value_groups: int) -> torch.Tensor:
    ...
    hidden_states = hidden_states[:, :, None, :, :].expand(batch_size, num_key_value_heads, num_key_value_groups, seq_len, head_dim)
    return hidden_states.reshape(batch_size, num_key_value_heads * num_key_value_groups, seq_len, head_dim)
...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        if self.sliding_window is not None:
            window_mask = torch.triu(causal_mask, diagonal=-(self.sliding_window - 1))
