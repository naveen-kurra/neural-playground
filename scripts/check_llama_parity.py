...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare exported LLaMA model.py against Hugging Face LLaMA.")
    parser.add_argument("--model-file", required=True, help="Path to generated LLaMA model.py")
...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("generated_llama_model", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to import generated module from {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generated_llama_model"] = module
...
def main() -> int:
    ...
    hf_model = HFLlamaForCausalLM(hf_config).eval()
    generated_config = generated.LlamaConfig(
        vocab_size=args.vocab_size,
        hidden_size=args.hidden_size,
        intermediate_size=args.intermediate_size,
        num_hidden_layers=args.layers,
    ...
    generated_model = generated.LlamaForCausalLM(generated_config).eval()
