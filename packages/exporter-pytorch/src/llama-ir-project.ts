...
import { exportLlamaIrToPyTorch } from "./llama-ir-export";
import {
  exportCheckpointPy,
  exportDataPy,
  exportEvalPy,
...
function exportLlamaModelYaml(spec: LlamaArchitectureSpec): string {
...
num_key_value_heads: ${spec.config.numKeyValueHeads}
head_dim: ${spec.config.headDim}
hidden_act: ${spec.config.hiddenActivation}
max_position_embeddings: ${spec.config.maxPositionEmbeddings}
rms_norm_eps: ${spec.config.rmsNormEpsilon}
rope_theta: ${spec.config.ropeTheta}
attention_bias: ${String(spec.config.attentionBias).toLowerCase()}
attention_dropout: ${spec.config.attentionDropout}
mlp_bias: ${String(spec.config.mlpBias).toLowerCase()}
tie_word_embeddings: ${String(spec.config.tieWordEmbeddings).toLowerCase()}
`;
}

function exportLlamaTrainYaml(training: TrainingConfig): string {
...
function exportLlamaConfigPy(): string {
...
function exportLlamaTrainModulePy(lossName: string): string {
  return `from __future__ import annotations
...
function exportLlamaScriptTrainPy(training: TrainingConfig): string {
  const optimizer = training.optimizer === "Custom" ? (training.optimizerCustomName || "custom_optimizer") : training.optimizer.toLowerCase();
...
from kurra_ai_cb.model import LlamaConfig, LlamaForCausalLM
from kurra_ai_cb.schedule import lr_scale
from kurra_ai_cb.train import train_step


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train exported LLaMA LM")
...
def build_model(cfg) -> LlamaForCausalLM:
    model_cfg = cfg.model
    return LlamaForCausalLM(
        config=LlamaConfig(
            vocab_size=model_cfg.vocab_size,
...
function exportLlamaReadme(spec: LlamaArchitectureSpec, training: TrainingConfig): string {
  return `# Exported LLaMA Project

This project was generated from Neural Playground using the LLaMA IR path.
...
- key/value heads: ${spec.config.numKeyValueHeads}
- activation: ${spec.config.hiddenActivation}
- rope theta: ${spec.config.ropeTheta}
...
function exportLlamaHookNotes(training: TrainingConfig): string {
