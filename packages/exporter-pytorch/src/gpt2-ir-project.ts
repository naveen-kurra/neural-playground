...
import { exportGPT2IrToPyTorch } from "./gpt2-ir-export";
import {
  exportCheckpointPy,
  exportDataPy,
  exportEvalPy,
...
function exportGPT2ModelYaml(spec: GPT2ArchitectureSpec): string {
  return `model_family: gpt2
...
num_hidden_layers: ${spec.config.numHiddenLayers}
num_attention_heads: ${spec.config.numAttentionHeads}
intermediate_size: ${spec.config.intermediateSize}
activation_function: ${spec.config.activationFunction}
embd_pdrop: ${spec.config.embdDropout}
attn_pdrop: ${spec.config.attnDropout}
resid_pdrop: ${spec.config.residDropout}
layer_norm_epsilon: ${spec.config.layerNormEpsilon}
scale_attn_weights: ${String(spec.config.scaleAttnWeights).toLowerCase()}
scale_attn_by_inverse_layer_idx: ${String(spec.config.scaleAttnByInverseLayerIdx).toLowerCase()}
reorder_and_upcast_attn: ${String(spec.config.reorderAndUpcastAttn).toLowerCase()}
tie_word_embeddings: ${String(spec.config.tieWordEmbeddings).toLowerCase()}
`;
}

function exportGPT2TrainYaml(training: TrainingConfig): string {
...
function exportGPT2ConfigPy(): string {
...
function exportGPT2TrainModulePy(lossName: string): string {
  return `from __future__ import annotations
...
function exportGPT2ScriptTrainPy(training: TrainingConfig): string {
  const optimizer = training.optimizer === "Custom" ? (training.optimizerCustomName || "custom_optimizer") : training.optimizer.toLowerCase();
...
from kurra_ai_cb.model import GPT2Config, GPT2LMHeadModel
from kurra_ai_cb.schedule import lr_scale
from kurra_ai_cb.train import train_step


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train exported GPT-2 LM")
...
def build_model(cfg) -> GPT2LMHeadModel:
    model_cfg = cfg.model
    return GPT2LMHeadModel(
        config=GPT2Config(
            vocab_size=model_cfg.vocab_size,
            n_positions=model_cfg.max_position_embeddings,
...
function exportGPT2Readme(spec: GPT2ArchitectureSpec, training: TrainingConfig): string {
  return `# Exported GPT-2 Project

This project was generated from Neural Playground using the GPT-2 IR path.

## Model Summary

- family: gpt2
- vocab size: ${spec.config.vocabSize}
- max positions: ${spec.config.maxPositionEmbeddings}
- hidden size: ${spec.config.hiddenSize}
- decoder blocks: ${spec.config.numHiddenLayers}
- attention heads: ${spec.config.numAttentionHeads}
- activation: ${spec.config.activationFunction}
- optimizer: ${training.optimizer === "Custom" ? training.optimizerCustomName || "custom_optimizer" : training.optimizer}
- loss: ${training.loss === "Custom" ? training.lossCustomName || "custom_loss" : training.loss}
...
function exportGPT2HookNotes(training: TrainingConfig): string {
  const lines = ["# Generated custom hook notes", "# Implement any referenced custom names in the exported project files."];
  if (training.loss === "Custom") {
    lines.push(`# Loss hook: implement '${training.lossCustomName || "custom_loss"}' in src/kurra_ai_cb/train.py:get_loss_function`);
  }
  if (training.optimizer === "Custom") {
    lines.push(`# Optimizer hook: implement '${training.optimizerCustomName || "custom_optimizer"}' in scripts/train.py:build_optimizer`);
  }
  if (training.activation === "Custom") {
    lines.push(`# Activation hook: implement '${training.activationCustomName || "custom_activation"}' in src/kurra_ai_cb/model.py:get_activation`);
  }
  return lines.join("\n") + "\n";
}
